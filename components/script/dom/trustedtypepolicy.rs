...
use crate::dom::bindings::reflector::{DomGlobal, reflect_dom_object, Reflector};
...
impl TrustedTypePolicy {
    ...
    fn create_trusted_type(&self, name: TrustedTypeName, value: DOMString) -> Fallible<TrustedResult> {
        ...
        let policy_value = self.get_trusted_type_policy_value(name, value)?;
    ...
    fn get_trusted_type_policy_value(
        ...
        let policy_value_result = match name {
            TrustedTypeName::TrustedHTML => {
                self.options.createHTML.clone().map(|callback| {
                    if let Ok(r) = callback.Call__(input, vec![], ExceptionHandling::Report) {
            ...
            TrustedTypeName::TrustedScript => {
                self.options.createScript.clone().map(|callback| {
                    if let Ok(r) = callback.Call__(input, vec![], ExceptionHandling::Report) {
                        TrustedResult::HTMLOrScript(r.expect("Failed to extract result"))
                    } else {
                        TrustedResult::Empty
...
impl TrustedTypePolicyMethods<crate::DomTypeHolder> for TrustedTypePolicy {
    ...
    fn CreateHTML(
        ...
        let result = self.create_trusted_type(TrustedTypeName::TrustedHTML, input);
    ...
    fn CreateScript(
        ...
        let result = self.create_trusted_type(TrustedTypeName::TrustedScript, input);
    ...
    fn CreateScriptURL(
        ...
        let result = self.create_trusted_type(TrustedTypeName::TrustedScriptURL, input);
