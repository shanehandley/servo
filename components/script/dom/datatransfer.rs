...
impl DataTransfer {
    ...
    pub fn new(window: &Window) -> DomRoot<DataTransfer> {
        ...
        reflect_dom_object(Box::new(DataTransfer::new_inherited(files, items)), window)
    }
...
impl DataTransferMethods for DataTransfer {
    ...
    fn ClearData(&self, format: Option<DOMString>) {
        warn!(
            ...
            format
                .clone()
                .unwrap_or(DOMString::from_string("none_provided".to_owned()))
        );
    ...
    fn Files(&self) -> DomRoot<FileList> {
        let file_list = FileList::new(&self.global().as_window(), self.items.get_files());
